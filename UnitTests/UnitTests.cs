...
#pragma warning disable CS1718 // Comparison made to same variable
            ...
            Assert.AreEqual(1 <= 2 || y, _converter.Convert(args, typeof(object), "1 <= 2 || y", CultureInfo.GetCultureInfo("de")));
            // +,- applied before ||
            try { _converter.Convert(args, typeof(object), "y || y + \"a\"", CultureInfo.GetCultureInfo("de")); Assert.Fail("|| is applied before +"); } catch (RuntimeBinderException ex) when (ex.Message == "Operator '||' cannot be applied to operands of type 'bool' and 'string'") { }
            try { _converter.Convert(args, typeof(object), "y - 3 || y", CultureInfo.GetCultureInfo("de")); Assert.Fail("|| is applied before -"); } catch(RuntimeBinderException ex) when (ex.Message == "Operator '-' cannot be applied to operands of type 'bool' and 'double'") { }
            // *,/ applied before ||
            try { _converter.Convert(args, typeof(object), "y * 3 || y", CultureInfo.GetCultureInfo("de")); Assert.Fail("|| is applied before *"); } catch (RuntimeBinderException ex) when (ex.Message == "Operator '*' cannot be applied to operands of type 'bool' and 'double'") { }
            try { _converter.Convert(args, typeof(object), "y / 3 || y", CultureInfo.GetCultureInfo("de")); Assert.Fail("|| is applied before /"); } catch (RuntimeBinderException ex) when (ex.Message == "Operator '/' cannot be applied to operands of type 'bool' and 'double'") { }
            ...
            Assert.AreEqual(1 <= 2 && y, _converter.Convert(args, typeof(object), "1 <= 2 && y", CultureInfo.GetCultureInfo("de")));
            // +,- applied before &&
            try { _converter.Convert(args, typeof(object), "x && x + \"a\"", CultureInfo.GetCultureInfo("de")); Assert.Fail("&& is applied before +"); } catch (RuntimeBinderException ex) when (ex.Message == "Operator '&&' cannot be applied to operands of type 'bool' and 'string'") { }
            try { _converter.Convert(args, typeof(object), "x - 3 && x", CultureInfo.GetCultureInfo("de")); Assert.Fail("&& is applied before -"); } catch (RuntimeBinderException ex) when (ex.Message == "Operator '-' cannot be applied to operands of type 'bool' and 'double'") { }
            // *,/ applied before &&
            try { _converter.Convert(args, typeof(object), "y * 3 && y", CultureInfo.GetCultureInfo("de")); Assert.Fail("&& is applied before *"); } catch (RuntimeBinderException ex) when (ex.Message == "Operator '*' cannot be applied to operands of type 'bool' and 'double'") { }
            try { _converter.Convert(args, typeof(object), "y / 3 && y", CultureInfo.GetCultureInfo("de")); Assert.Fail("&& is applied before /"); } catch (RuntimeBinderException ex) when (ex.Message == "Operator '/' cannot be applied to operands of type 'bool' and 'double'") { }
            ...
            // ^ applied before &&:
            try { _converter.Convert(args, typeof(object), "x ^ true && true ? x : 0", CultureInfo.GetCultureInfo("de")); Assert.Fail("3 ^ true should return null, which cannot be AND-ed"); } catch (RuntimeBinderException ex) when (ex.Message == "Cannot convert null to 'bool' because it is a non-nullable value type") { }
